...
const getAPIEndpoint = (machineId) => `/api/v1/business-owner/machine/${machineId}/create-networked`;

describe('test createNetworkedMachineByOffline api', () => {
    let user, business, store, batch, device, token, machine, configurations;
    ...
    describe('when authorization is failed', function () {
        itShouldCorrectlyAssertTokenPresense(assertPostResponseError, () => getAPIEndpoint(machine.id));
    ...
    describe('when device is wrong', () => {
        ...
        it('should respond 409 if the device is already paired', async () => {
            ...
            await assertPostResponseError({
                url: getAPIEndpoint(machine.id),
                body: {
                    name: 'Aurora',deviceId: deviceWrong.id,
                },
                token: generateToken({
                    id: user.id,
                }),
    ...
    describe('when machine is wrong', () => {
        it('should return 404 if machine is not found', async () => {
            await assertPostResponseError({
                url: getAPIEndpoint(5674322),
                body: {
                    name: 'Aurora',
                    deviceId: device.id,
                },
        ...
        it('should return 409 if machine with the same name already exist', async () => {
            ...
            await factory.create(FACTORIES_NAMES.machineConfiguration, mockConfigurationsBody);
    ...
    describe('when networked machine is created', () => {
        it('should return status 201 and machine with the same id', async () => {
            const mockConfigurationsBody = {
                ...mockMachineFeatureConfig({ PennyID: device.name }),
                ...mockMachineModelConfig({ PennyID: device.name }),
                ...mockMachineProgrammingWasherConfig({ PennyID: device.name }),
            ...
            expect(body).to.have.property('machine').to.have.property('id').to.be.eql(machine.id);
        });
        ...
        it('should create a washer machine', async () => {
            ...
            expect(body.machine).to.have.property('id').to.be.a('number').to.eql(machine.id);
