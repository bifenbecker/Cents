 ...
 */
function formatIndividualLineItem(lineItem) {
    const item = {};
    const quantity =
...
async function createUberDelivery(payload) {
    try {
        if (!payload.uberToken) {
            return payload;
        }
        const newPayload = payload;
        const { serviceOrder, transaction, storeCustomer, orderDelivery } = payload;

        const { orderItems } = serviceOrder;
        const referenceItems = orderItems.map((item) => item.referenceItems);
        const lineItems = referenceItems.map((item) => item[0].lineItemDetail);
        const formattedLineItems = formatOrderItems(lineItems);
        ...
        const params = {
            estimate_id: orderDelivery.thirdPartyDeliveryId,
            pickup_at: orderDelivery.deliveryWindow[0],
            external_order_id: `${serviceOrder.orderCode}`,
            order_items: formattedLineItems,
            // TODO: update pickup obj to reflect customer address.
            pickup: {
                store_id: store.uberStoreUuid,
                instructions: `Pickup order number ${serviceOrder.orderCode} for ${storeCustomer.firstName} ${storeCustomer.lastName}`,
                external_store_id: `${store.id}`,
            },
            // TODO: update dropoff obj to reflect store address.
            dropoff: {
                address: {
                    place: {
            ...
            courier_tip: newPayload.courierTip ? Number(newPayload.courierTip * 100) : null,
            external_user_id: `${storeCustomer.id}`,
            order_summary: {
                currency_code: 'USD',
