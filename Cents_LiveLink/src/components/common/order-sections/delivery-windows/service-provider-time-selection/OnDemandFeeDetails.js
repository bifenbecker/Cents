...
import {getSubsidyInCents} from "./utils.js";
...
const OnDemandFeeDetails = props => {
  const {
    ...
    currentUberEstimate,
    settings,
    currentDoorDashEstimate,
    orderType,
  } = props;
  const {subsidyInCents, returnOnlySubsidyInCents} = settings;
          ...
          <Flex flexDirection="row" py="12px" justifyContent="space-between">
            <Flex flexDirection="column">
              ...
              <Text {...styles.deliveryFee}>
                {totalCost
                  ? toDollars(Math.max(0, totalCost - finalSubsidyInCents) / 100)
                  : null}
